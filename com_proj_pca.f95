	...
	INTEGER,INTENT(in) :: n
	REAL :: mass
	character(16)crap,atype,restype,chain,aat,protname,crap2
	character(128)line
	...
	!do i=1,3 !For Anton trajectory
	!  read(13,'(A)')line
	!end do
	do i=1,natoms
	  read(13,*)crap,anum,atype,restype,rnum !Assuming a single chain
	  !read(13,*)crap,anum,atype,restype,chain,rnum 
	  !read(13,*)crap,anum,atype,restype,chain,crap2 !For Anton trajectory
	...
	DOUBLE PRECISION :: sumx(n),sumy(n),sumz(n),summa(n),R(3*n,3*n),RT(3*n,3*n),pi,rcomx(nfrs),rcomy(nfrs)
	DOUBLE PRECISION :: avgx(n),avgy(n),avgz(n),c(3*n,3*n),work(3*n*3*n),Atest(3*n,3*n),rcomz(nfrs)
	...
	DOUBLE PRECISION :: xixavg(n),xiyavg(n),xizavg(n),xixref(n,nfrs),xiyref(n,nfrs),xizref(n,nfrs),PCfluct(3*n)
	...
	OPEN(unit=99,file='format',status='old')
	READ(99,'(A)')tipo
	CLOSE(99)
	IF(tipo .EQ. 'xyz')THEN
	  !read from trajectory (xyz)
	  open(unit=11,file=trim(protname)//'.xyz',status='old')
	  ...
	  do k=1,nfrs
	    ...
	    do j=1,n
	      read(11,*)aa,rx(j,k),ry(j,k),rz(j,k)
	...
	IF(ion .EQ. 0)THEN !No mass-weighting
	  OPEN(unit=3,file='com_covariance_matrix.dat',status='unknown')
	...
	if(minval(eiglam).lt. -0.000000001) then
	  write(*,*)"lambda neg"
	  write(40,*)0
	else
	  write(*,*)"pos. def."
...
!	  write(*,*)eiglamO(i),order(i)
	...
	open(unit=10,file="com_Rmatrix",status='unknown')
	open(unit=11,file="com_RTmatrix",status='unknown')
	do i=1,3*n
	  do j=1,3*n
	    write(10,*)R(i,j)
	    write(11,*)RT(i,j)
	...
	zfluct=0.0
	PCfluct=0.0
	OPEN(unit=26,file='com_PCmavg_ref.dat',status='unknown')
	OPEN(unit=30,file='com_PCavg_ref.dat',status='unknown')
	DO a=1,3*n
	  WRITE(aa,*)a
	  aa=adjustl(aa)
	  OPEN(unit=25,file='PC_'//TRIM(aa)//'.dat',status='unknown')
	...
	CLOSE(26)
	CLOSE(30)
        OPEN(unit=26,file='com_PC_flucts',status='unknown')
        OPEN(unit=27,file='com_PC_dev',status='unknown')
        DO a=1,3*n
          WRITE(26,*)xfluct(a),yfluct(a),zfluct(a)
	  WRITE(27,*)DSQRT(PCfluct(a)-PCavg(a)**2)
        END DO
        CLOSE(26)
	CLOSE(27)
	!Enough of that nonsense

	open(unit=99,file="com_covar_eig",status='unknown')
	...
	DOUBLE PRECISION :: proj(n,3*n),RT(3*n,3*n),R(3*n,3*n),LMLmag(n,3*n),c(3*n,3*n),rmsf(n),var(nfrs)
        DOUBLE PRECISION :: projx(n,3*n),projy(n,3*n),projz(n,3*n),LML(3*n,3*n),chk(3*n),eiglam(3*n),La(3*n)
	DOUBLE PRECISION :: biglist(3*n,nfrs),QINV(n,n),rx(n,nfrs),ry(n,nfrs),rz(n,nfrs),summa(3*n),avbl,fluct(3*n)
	...
	La=0.0
	fluct=0.0
        !OPEN(unit=20,file='avbl',status='old')
        !READ(20,*)avbl
        !CLOSE(20)
	WRITE(*,*)'Projections'
	OPEN(unit=20,file='com_Rmatrix',status='old')
        DO i=1,3*n
          DO a=1,3*n
            READ(20,*)R(i,a)
          END DO
        END DO
        CLOSE(20)
	OPEN(unit=20,file='com_RTmatrix',status='old')
        DO a=1,3*n
          DO i=1,3*n
            READ(20,*)RT(a,i)
          END DO
        END DO
        CLOSE(20)
	OPEN(unit=20,file='com_covariance_matrix.dat',status='old')
        DO i=1,3*n
          DO j=1,3*n
            READ(20,*)c(i,j)
          END DO
        END DO
        CLOSE(20)
	OPEN(unit=20,file='com_covar_eig',status='old')
        DO a=1,3*n
          READ(20,*)eiglam(a)
        END DO
        CLOSE(20)
	OPEN(unit=20,file='com_biglist.dat',status='old')
        DO k=1,nfrs
	...
	DO a=1,3*n
	  ...
	  CLOSE(26)
        ...
        DO a=1,3*n
          ...
          CLOSE(24)
        END DO
	!Find the length of each mode:
	DO a=1,3*n
	  DO i=1,n
	    La(a)=LMLmag(i,a)+La(a)
	  END DO
	END DO
	summa=0.0
        OPEN(unit=24,file='gating_bonds_pca.dat',status='unknown')      
        DO a=1,3*n
          DO i=1,n
            summa(a)=summa(a)+LMLmag(i,a)/avbl
          END DO
          WRITE(*,*)"Gating Bonds for mode" ,a," :",summa(a)
          WRITE(24,*)a,summa(a)
        END DO
        CLOSE(24)
        ...
        END DO
        CLOSE(24)
	!Find fluctuations relative to mode length
	OPEN(unit=24,file='PC_dev',status='old')
	DO a=1,3*n
	  READ(24,*)fluct(a)
	END DO
	CLOSE(24)
	DO a=1,3*n
	  fluct(a)=fluct(a)/La(a)
	END DO
	OPEN(unit=24,file='PC_reldev',status='unknown')
	DO a=1,3*n
	  WRITE(24,*)fluct(a)
	END DO
	CLOSE(24)
	...
	subroutine lproject(n,nfrs)!,nmol)
	IMPLICIT NONE
	INTEGER :: n,nfrs,nmol
	INTEGER :: i,j,k,a,counter,countery,counterz,maxframe(1),minframe(1)
	DOUBLE PRECISION :: xix(n-1,nfrs),xiy(n-1,nfrs),xiz(n-1,nfrs),xixavg(n-1),xiyavg(n-1),xizavg(n-1)
	...
	WRITE(*,*)'moving onto the LE4PD portion of the program.'

	Q=0.0
	QINV=0.0
	!Covariance matrix from LE4PD
	OPEN(unit=24,file='Qmatrix',status='old')
	DO i=1,n-nmol
	  DO a=1,n-nmol
	    READ(24,*)Q(i,a)
	  END DO
	...
	OPEN(unit=24,file='QINVmatrix',status='old')
	DO a=1,n-nmol
	  DO i=1,n-nmol
	    READ(24,*)QINV(a,i)
	  END DO
	...
	!Calculate and write the trajectories of some representative normal modes to file:
	xix=0.0
	xiy=0.0
	xiz=0.0
	xim=0.0
	...
	do k=1,nfrs
	  ...
	  IF(nmol .EQ. 1)THEN
	    do j=1,n-nmol
	      ...
	      lz(j,k)=rz(j+1,k)-rz(j,k)
	    END DO
	  ELSE !Only dimers for now
	    do j=1,n-nmol
	      IF(j .LE. (n/2)-1)THEN !Residues 1-99 for protease (bonds 1-98)
	        lx(j,k)=rx(j+1,k)-rx(j,k)
	        ly(j,k)=ry(j+1,k)-ry(j,k)
	        lz(j,k)=rz(j+1,k)-rz(j,k)
	      ELSE !Bonds 99-197
	        lx(j,k)=rx(j+2,k)-rx(j+1,k)
	        ly(j,k)=ry(j+2,k)-ry(j+1,k)
	        lz(j,k)=rz(j+2,k)-rz(j+1,k)
	      END IF 
	    END DO
	  END IF
	!skip 8 lines
	...
	OPEN(unit=26,file='ximavg.dat',status='unknown')
	OPEN(unit=30,file='xiavg.dat',status='unknown')
	DO a=1,n-nmol
	  WRITE(aa,*)a
	  aa=adjustl(aa)
	  IF(a .LE. 10)OPEN(unit=25,file='xi_'//TRIM(aa)//'.dat',status='unknown')
	  IF(a .LE. 10)OPEN(unit=27,file='xix_'//TRIM(aa)//'.dat',status='unknown')
	  IF(a .LE. 10)OPEN(unit=28,file='xiy_'//TRIM(aa)//'.dat',status='unknown')
	  IF(a .LE. 10)OPEN(unit=29,file='xiz_'//TRIM(aa)//'.dat',status='unknown')
	  DO k=1,nfrs
	    DO j=1,n-nmol
	      xix(a,k)=QINV(a,j)*lx(j,k)+xix(a,k)
	      xiy(a,k)=QINV(a,j)*ly(j,k)+xiy(a,k)
	      xiz(a,k)=QINV(a,j)*lz(j,k)+xiz(a,k)
	    end do
	    xi(a,k)=xix(a,k)+xiy(a,k)+xiz(a,k)
	    xim(a,k)=(xix(a,k)**2+xiy(a,k)**2+xiz(a,k)**2)**.5
	    xixavg(a)=xixavg(a)+xix(a,k)
	    xiyavg(a)=xiyavg(a)+xiy(a,k)
	    xizavg(a)=xizavg(a)+xiz(a,k)
	    ximavg(a)=ximavg(a)+xim(a,k)
	    xiavg(a)=xiavg(a)+xi(a,k)
	    xifluct(a)=xifluct(a)+xi(a,k)**2
            xfluct(a)=xfluct(a)+xix(a,k)**2
            yfluct(a)=yfluct(a)+xiy(a,k)**2
            zfluct(a)=zfluct(a)+xiz(a,k)**2
	    IF(a .LE. 10)WRITE(25,*)0.2*k,xi(a,k)
	    IF(a .LE. 10)WRITE(27,*)0.2*k,xix(a,k)
	    IF(a .LE. 10)WRITE(28,*)0.2*k,xiy(a,k)
	    IF(a .LE. 10)WRITE(29,*)0.2*k,xiz(a,k)
	  END DO
	  IF(a .LE. 10)CLOSE(25)
	  IF(a .LE. 10)CLOSE(27)
	  IF(a .LE. 10)CLOSE(28)
	  IF(a .LE. 10)CLOSE(29)
	  xixavg(a)=xixavg(a)/REAL(nfrs)
	  xiyavg(a)=xiyavg(a)/REAL(nfrs)
	  xizavg(a)=xizavg(a)/REAL(nfrs)
	  ximavg(a)=ximavg(a)/REAL(nfrs)
	  xifluct(a)=xifluct(a)/REAL(nfrs)
          xfluct(a)=xfluct(a)/REAL(nfrs)
          yfluct(a)=yfluct(a)/REAL(nfrs)
          zfluct(a)=zfluct(a)/REAL(nfrs)
	  WRITE(26,*)a,ximavg(a)
	  xiavg(a)=xiavg(a)/REAL(nfrs)
	  WRITE(30,*)a,ABS(xiavg(a))
	END DO
	CLOSE(26)
	CLOSE(30)
	
        OPEN(unit=26,file='xi_flucts',status='unknown')
        OPEN(unit=27,file='xi_relflcut',status='unknown')
        DO a=1,n-nmol
	  WRITE(aa,*)a
	  aa=adjustl(aa)
	  OPEN(unit=10,file="lml_"//TRIM(aa)//".dat",status='old')
	  lml=0.0
	  DO i=1,n-nmol
	    dummy=0.0
	    READ(10,*)dummy !LML of bond i along the ath mode
	    lml(a)=dummy+lml(a) !Summing to find the mode length
	  END DO
	  CLOSE(10)
          WRITE(26,*)xfluct(a),yfluct(a),zfluct(a)
	  WRITE(27,*)DSQRT(xifluct(a)-xiavg(a)**2)/lml(a)
        END DO
        CLOSE(26)
	CLOSE(27)
	!Enough of that nonsense
